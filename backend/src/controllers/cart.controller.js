...
async function postCart(req, res) {
    ...
    if (!qty || qty <= 0) {
        return res.status(400).json({ error: { code: 'BAD_REQUEST', message: 'Quantity must be positive' } });
    }
...
async function checkout(req, res, next) {
    ...
    try {
        ...
        if (!cart || cart.items.length === 0) {
            return res.status(400).json({ error: { code: 'BAD_REQUEST', message: 'Cart empty' } });
        }
        ...
        for (const it of cart.items) {
            const prod = await Product.findOneAndUpdate(
                { _id: it.product._id, available: true },
                { $set: { available: false } },
                { new: true, session }
            );
            if (!prod) {
...
async function putCart(req, res, next) {
    try {
        ...
        if (!Array.isArray(items)) {
            return res.status(400).json({ error: { code: 'BAD_REQUEST', message: 'items array required' } });
        }
...
module.exports = { getCart, postCart, deleteCartItem, checkout, putCart };
